	...
	public void startGrassPlacing (TerrainData terrainData, int[,] colorMap, int grassDensity, int patchDetail, float waterTop, float fieldTop, int cityGround)
	{
		...
		for (int i = 0; i < grassDensity; i++)
		{
			for (int j = 0; j < grassDensity; j++)
			{
				...
				if (height > waterTop+0.0001f && height < fieldTop && steepness < 24f && colorMap[i*(terrainData.detailWidth/ colorMap.GetLength(0)), j*(terrainData.detailHeight/ colorMap.GetLength(0))] != cityGround) {
					map [i, j] = 16;
				}
			}
		}
